...
parameters {
string(name: 'parameter', defaultValue: 'clean package', description: 'This is used to clean package')
}
stages {
stage('vcs') {
steps {
git url: 'https://github.com/Danish-Ansarii/game-of-life.git', branch: 'master'
}
}
stage('build') {
steps {
sh 'export PATH="/usr/lib/jvm/java-1.8.0-openjdk-amd64/bin:$PATH" && mvn ${params.parameter}'
}
}
stage('SonarQube analysis') {
